...
namespace Vulkyrie::Renderer {
    OpenGLComputeShader::OpenGLComputeShader(const std::filesystem::path &computeShaderPath) : _computeShaderPath(computeShaderPath) {
        _shaderProgram = LoadAndCompile();
    }
    ...
    inline void OpenGLComputeShader::Use() const {
        glUseProgram(_shaderProgram);
    }

    u32 OpenGLComputeShader::Reload() {
        // Create a new compute shader program.
        u32 newShaderProgram = LoadAndCompile();
    ...
    u32 OpenGLComputeShader::LoadAndCompile() {
