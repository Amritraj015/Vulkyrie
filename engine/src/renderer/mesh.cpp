...
namespace Vulkyrie::Renderer {
    Ref<Mesh> Mesh::Create(Vulkyrie::Core::GraphicsAPI api, std::vector<Vertex> &&vertices, std::vector<u32> &&indices, std::vector<std::pair<MeshTextureType, Ref<Texture2D>>> &&textures) {
        switch (api) {
