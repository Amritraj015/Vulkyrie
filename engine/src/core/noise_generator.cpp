...
namespace Vulkyrie::Core {
    std::vector<f32> GeneratePerlinNoiseMap(const NoiseMapSpecification &specification) {
        ...
        glm::vec2 offset = glm::mod(specification.Offset, 100000.0f);
        ...
        for (size_t y = 0; y < height; ++y) {
            ...
            for (size_t x = 0; x < width; ++x) {
                ...
                const auto baseX = (static_cast<f32>(x) - halfWidth) * invScale;
                const auto baseY = (static_cast<f32>(y) - halfHeight) * invScale;
                ...
                for (size_t i = 0; i < octaves; ++i) {
                    ...
                    const f32 sampleX = baseX * frequency + octaveOffsets[i].x + offset.x;
                    const f32 sampleY = baseY * frequency + octaveOffsets[i].y + offset.y;
