...
namespace Vulkyrie {
     ...
     */
    inline constexpr f32 VE_MACHINE_EPSILON = std::numeric_limits<f32>::epsilon();
    inline constexpr f32 VE_DECIMAL_MAX = std::numeric_limits<f32>::max();
    inline constexpr f32 VE_DECIMAL_MIN = std::numeric_limits<f32>::min();
    inline constexpr f32 VE_DECIMAL_LOWEST = std::numeric_limits<f32>::lowest();
