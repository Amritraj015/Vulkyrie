...
TEST_CASE("TransientPool - two disjoint lifetimes in the same frame share one resource", "[transientpool]") {
    ...
    const auto first = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 2 });
    const auto second = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 3, .LastUse = 5 });
...
TEST_CASE("TransientPool - overlapping lifetimes in the same frame get distinct resources", "[transientpool]") {
    ...
    const auto first = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 5 });
    const auto second = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 2, .LastUse = 7 });
...
TEST_CASE("TransientPool - a third request slots into whichever earlier entry is free", "[transientpool]") {
    ...
    const auto a = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 5 });
    const auto b = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 2, .LastUse = 7 });
    const auto c = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 6, .LastUse = 8 });
...
TEST_CASE("TransientPool - a different descriptor never reuses another descriptor's entry", "[transientpool]") {
    ...
    const auto first = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 1 });
    const auto second = fixture.Pool.Acquire(OTHER_TEXTURE, LifeTime{ .FirstUse = 2, .LastUse = 3 });
...
TEST_CASE("TransientPool - ResetFrame lets a new frame reuse an entry regardless of last frame's interval", "[transientpool]") {
    ...
    const auto frame1 = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 10, .LastUse = 20 });
    ...
    const auto frame2 = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 1 });
...
TEST_CASE("TransientPool - buffers reuse the same way images do", "[transientpool]") {
    ...
    const auto first = fixture.Pool.Acquire(SOME_BUFFER, LifeTime{ .FirstUse = 0, .LastUse = 2 });
    const auto second = fixture.Pool.Acquire(SOME_BUFFER, LifeTime{ .FirstUse = 3, .LastUse = 5 });
...
TEST_CASE("TransientPool - TrimUnused ages out entries untouched for more than the threshold", "[transientpool]") {
    ...
    [[maybe_unused]] const auto acquired = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 1 });
...
TEST_CASE("TransientPool - TrimUnused keeps an entry that is still being reacquired", "[transientpool]") {
    ...
    for (int i = 0; i < 5; ++i) {
        fixture.Pool.ResetFrame();
        [[maybe_unused]] const auto reacquired = fixture.Pool.Acquire(SOME_TEXTURE, LifeTime{ .FirstUse = 0, .LastUse = 1 });
        fixture.Pool.TrimUnused(/*unusedFrameThreshold=*/2);
    }
